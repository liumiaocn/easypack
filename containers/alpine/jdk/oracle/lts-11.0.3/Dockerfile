...
ARG VER_JAVA_MAJOR=11.0.3
ARG DIR_JAVA_BASE=/usr/local/share/java
ARG DIR_JAVA_HOME=${DIR_JAVA_BASE}/jdk-11.0.3
ARG ARC_MODE=x64
ARG VER_GLIBC=2.29-r0
ARG GLIBC_DOWNLOAD_URL=https://github.com/sgerrand/alpine-pkg-glibc/releases/download/${VER_GLIBC}

COPY jdk-${VER_JAVA_MAJOR}_linux-${ARC_MODE}_bin.tar.gz /tmp

RUN mkdir -p ${DIR_JAVA_HOME} && \
    tar zxf /tmp/jdk-${VER_JAVA_MAJOR}_linux-${ARC_MODE}_bin.tar.gz -C ${DIR_JAVA_BASE} && \
    ...
    sleep 2 &&\
    rm -rf /tmp/* /var/cache/apk/* /tmp/jdk-${VER_JAVA_MAJOR}_linux-${ARC_MODE}_bin.tar.gz &&\
